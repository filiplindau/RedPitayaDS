...
class RedPitaya_control(object):
	...
	def sendReceive(self, msg):
		...
		try:
			...
			print "sendReceive timeout, retrying"
			rep = self.sock.recv(70000)
		return rep
...
#		self.redPitayaData.triggerDelayTime = (self.redPitayaData.recordLength / 2.0 - trigDelay) / self.redPitayaData.sampleRate
	...
	def setGenOutEnable(self, channel):
		if (channel == 1):
			with self.lock:
				msg = ''.join(('genOutputEnableCh1')) 
				self.sendReceive(msg)
		elif (channel == 2):
			with self.lock:
				msg = ''.join(('genOutputEnableCh2')) 
				self.sendReceive(msg)
		else:
			raise ValueError(''.join(('Wrong channel ', str(channel), ', use 1, or 2')))

	def setGenFrequency(self, channel, freq):
		if (channel == 1):
			with self.lock:
				msg = ''.join(('genFrequencyCh1:',str(freq))) 
				self.sendReceive(msg)
		elif (channel == 2):
			with self.lock:
				msg = ''.join(('genFrequencyCh2:',str(freq))) 
				self.sendReceive(msg)
		else:
			raise ValueError(''.join(('Wrong channel ', str(channel), ', use 1, or 2')))
...
#        self.lock.release()
	...
	#				if True:
		...
		#			print "Not triggered"
				...
				print "updateWaveforms timeout, exiting read"
